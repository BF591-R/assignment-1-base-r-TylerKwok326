...
summarize_matrix <- function(x, na.rm = FALSE) {
  return(df <- data.frame(
    mean = summarize_rows(x, function(row) mean(row, na.rm = TRUE)), 
    stdev = summarize_rows(x, function(row) sd(row, na.rm = TRUE)),
    median = summarize_rows(x, function(row) median(row, na.rm = TRUE)),
    min = summarize_rows(x, function(row) min(row, na.rm = TRUE)),
    max = summarize_rows(x, function(row) max(row, na.rm = TRUE)),
    num_lt_0 = summarize_rows(x, function(row) {
      sum(less_than_zero(row[!is.na(row)]))  
    }),
    num_btw_1_and_5 = summarize_rows(x, function(row) {
      sum(is_between(row[!is.na(row)], 1, 5))  
    }),
